...
export default function Chat({ user }) {
    ...
    const startChat = async (selectedUser) => {
        try {
            const res = await api.post("/chats", { recipientId: selectedUser.id });
            setCurrentChat(res.data.chat || res.data);
        } catch (err) {
    ...
    return (
        <div className="flex h-screen gap-4 p-4 bg-green-50 dark:bg-green-900 transition-colors">
            <div className="w-1/3 overflow-y-auto space-y-2">
                {users.map((u) => (
                    <UserCard key={u.id} user={u} onClick={startChat} />
                ))}
            </div>
            <div className="flex-1">
                {currentChat ? (
                    <ChatWindow chatId={currentChat.id} userId={user.id} socket={socket} />
                ) : (
                    <div className="h-full flex items-center justify-center text-green-800 dark:text-green-200 opacity-50">
                ...
                <div className="flex-1 overflow-y-auto space-y-3">
                    {conversations.map((conv) => (
                        <motion.div
                            ...
                            className={`cursor-pointer rounded-xl p-3 transition-all shadow-sm ${activeConv?.id === conv.id
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                            {messages.map((msg, idx) => (
                                <motion.div
                                    ...
                                    className={`max-w-lg p-3 rounded-xl shadow-md ${msg.senderId === user.id
