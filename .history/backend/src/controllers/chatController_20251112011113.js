    ...
    try {
        ...
        const chatsWithLastMessage = await Promise.all(chats.map(async chat => {
            ...
            const unreadCount = await Message.count({
                where: { chatId: chat.id, isRead: false, senderId: { [Op.ne]: userId } },
            });
            return { ...chat.toJSON(), latestMessage: latest || null, unreadCount };
        }));
...
export async function getMessages(req, res) {
    try {
        ...
        if (unreadMessages.length > 0) {
            ...
            // Emit read receipts
            unreadMessages.forEach(m => io.to(`user_${m.senderId}`).emit("message_read", { chatId, messageId: m.id }));
        }
...
export async function sendMessage(req, res) {
    try {
        ...
        if (!recipientId || !text?.trim()) return res.status(400).json({ message: "Recipient and text required" });
